...
async def wait_bot_reply(tg, bot, after_id, target, timeout=600, g=None):
    """Espera o resultado FINAL da mensagem mais nova deste número. Ignora erro antigo."""
    deadline = asyncio.get_event_loop().time() + timeout
    last_logged = None

    while asyncio.get_event_loop().time() < deadline:
        if g and await group_has_feita_for_target(g, tg, target):
            log(f"Grupo já Feita para {target} — trata como APROVADA")
            return "approved", "feita manual no grupo"
        ...
        if newest_id:
            ...
            if text.strip():
                ...
                if stale and kind == "approved":
                    return kind, text
                if stale and kind == "idle" and re.search(
                    r"Aguardando checkout", text or "", re.I
                ):
                    state, hint = await bot_state_for_target(tg, bot, target)
                    if state == "approved":
                        log(f"APROVADA detectada após checkout parado ({target})")
...
async def cancel_non_claro_open_orders(g, tg, open_orders):
    """Libera fila: pedidos VIVO/TIM/etc. não são do bot Linkclaro."""
    changed = False
    for row in open_orders:
        ...
        log(
            f"Pedido {pid} operadora {payload_operator(payload) or '?'} — "
            "worker Claro não processa (cancelando para liberar fila)"
        )
...
async def reconcile_open_orders(g, tg, bot, open_orders):
    ...
    for row in open_orders:
        ...
        payload = row["payload"]
        msg_id = row["msg_id"]
        if await pedido_ja_feita(g, tg, pid, anchor_msg_id=msg_id):
            log(f"Reconcile: {pid} já Feita no grupo — limpando fila")
            if pid == (load_current_pedido() or {}).get("pedido_id"):
                clear_current_pedido()
            changed = True
            continue
        target = payload_target(payload)
...
async def acquire_order(g, tg, bot):
    ...
    if current:
        ...
        if pid and payload and await pedido_still_open(g, tg, pid, anchor_msg_id=msg_id):
            if not is_claro_payload(payload):
                log(f"Pedido atual {pid} não é Claro — cancelando")
                await cancel_order_in_group(g, tg, pid, log=log, anchor_msg_id=msg_id)
                clear_current_pedido()
            else:
    ...
    if await cancel_non_claro_open_orders(g, tg, open_orders):
        open_raw = await list_open_orders(g, tg)
        open_orders = [{"pedido_id": p, "payload": pay, "msg_id": mid} for p, pay, mid in open_raw]
    if len(open_orders) > 1:
        if await reconcile_open_orders(g, tg, bot, open_orders):
            open_raw = await list_open_orders(g, tg)
            open_orders = [{"pedido_id": p, "payload": pay, "msg_id": mid} for p, pay, mid in open_raw]
    open_orders = [o for o in open_orders if is_claro_payload(o.get("payload"))]
...
async def process_one_order(g, tg, bot, payload, pedido_id, max_cycles=50):
    ...
    if not is_claro_payload(payload):
        log(f"Operadora não-Claro ({payload}) — cancelando pedido {pedido_id}")
        await cancel_order_in_group(
            g, tg, pedido_id, log=log, anchor_msg_id=current_msg_id(pedido_id)
        )
        clear_current_pedido()
    ...
    if state == "progress":
        ...
        kind, text = await wait_bot_reply(tg, bot, 0, target, timeout=600, g=g)
        if kind == "approved":
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        kind, text = await wait_bot_reply(tg, bot, sent.id, target, timeout=600, g=g)
        action = await _apply_terminal(g, tg, payload, pedido_id, target, kind, text, error_state)
        if action == "retry":
            continue
        return action
    ...
    try:
        while True:
            if pending_confirm:
                ...
                if await pedido_ja_feita(
                    g, tg, pedido_id_retry, anchor_msg_id=msg_id_retry
                ):
                    pending_confirm = None
                    clear_current_pedido()
                    log(f"Pedido {pedido_id_retry} já Feita (manual) — próximo")
                    continue
            ...
            if result == "skipped_operator":
                log("Pedido não-Claro — próximo")
                await asyncio.sleep(3)
                continue
