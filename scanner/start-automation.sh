...
if curl -sf http://127.0.0.1:3000/health >/dev/null 2>&1; then
  echo "Automação já online em http://127.0.0.1:3000 (mate o processo para reiniciar com HEADLESS diferente)"
  exit 0
fi
...
$TMUX send-keys -t "$SESSION:0.0" "cd \"$DIR\" && BYPASS_3DS=true HEADLESS=$HEADLESS SKIP_CS_CAPTURE=1 node server.js 2>&1 | tee /tmp/automation-server.log" C-m
